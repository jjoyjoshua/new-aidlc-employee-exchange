 ...
 *
 * It depends on `zod` and nothing else (ADR-002). It describes what crosses the wire; the rules
 * live in `apps/api/src/domain`. Nothing here may import from `apps/api` or `apps/ui`, and
 * `eslint.config.mjs` enforces that — the browser reaching `apps/api` would put a path between
 * browser code and the service-role key (ADR-001).
 */
