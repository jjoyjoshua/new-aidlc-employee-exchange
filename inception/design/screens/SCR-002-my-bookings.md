...
| **Traces to**   | REQ-003, REQ-009, REQ-010, REQ-024, REQ-028, REQ-036, NFR-001, NFR-004, NFR-008                                                         |
...
| 4   | **REQ-036 refreshes this list when the window regains focus — nothing says what Priya sees when that refresh removes a row.** The requirement exists so she stops looking at a booking an admin already cancelled (REQ-014) or that her deactivation cancelled (BR-001.18). But no `ST-##` covers the outcome: an Upcoming row silently disappears, or silently moves to Past as **Cancelled**, with no explanation. ST-09 is not this — it covers a collision discovered *inside* the cancel dialog, when Priya is already acting. This is the passive case, on the screen whose whole job is answering "do I have a desk?", where a booking vanishing unannounced is exactly what teaches someone to distrust the tool. The cancellation email (REQ-024) arrives, but may be hours behind her looking at the screen. | REQ-036 vs PRIN-1, PRIN-5 | designer | **Open — raised 2026-09-10** by `/ux` while codifying REQ-036, and deferred by the designer the same day in favour of the two tidy-ups in this PR. Not a blocker: REQ-036 is implementable as written and the screen is not wrong today, since without the refresh the row simply stays stale. It becomes a gap the moment REQ-036 is built. Likely shape is a new state — the refreshed list carrying a quiet, dismissible line naming what changed (*"A-01 on Wed 9 Sep was cancelled by your office admin."*), which would reuse the actor-naming wording BR-001.20 already settled for push. That is a frame to draw, so it belongs to a design pass, not to this one |

Rows 1-3 are resolved and none changed BRD-001. Row 3 was opened and closed on
2026-09-08 by the hi-fi build; it changed `tokens.css`, not BRD-001. **Row 4 is open**
and is the only unresolved question on this screen.
