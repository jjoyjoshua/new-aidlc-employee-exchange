...
| **Traces to**   | REQ-006, REQ-007, REQ-008, REQ-017, REQ-023, REQ-034, REQ-035, NFR-001, NFR-004, NFR-008                                                                      |
...
- **Non-colour signalling:** Available, Taken and Selected each carry an icon **and** the word. A weekend chip says **Closed**; an out-of-range date says **Too far ahead**. Nothing on this screen is distinguishable by colour alone (NFR-008) — which matters most here, where "grey means you can't have it" would otherwise carry five different meanings
...
| A "your usual" hint on the last-booked desk, but no auto-selection                                    | A-2 predicts habitual desks, and this is the cheapest possible expression of it — a label, not a feature. Auto-selecting would pre-commit her to a desk she may not want, and it is not in any requirement                                                                                    | Auto-selecting the usual desk (invents behaviour); a favourites system (a requirement nobody wrote). **Accepted 2026-09-07 (Joy Joshua, PO/BA)** — derived from her own booking history, so no new data and no new stored preference. Codified 2026-09-10 as **REQ-034** (Could) — the priority is the PO’s, and it is the first thing to drop if the release tightens |
| The date strip shows **three** days at 360px, not seven | Measured, not chosen. The 360 content column is 328px and the strip card keeps 296px inside its padding; two paging arrows take 80px. At the established 76px chip only **two** days fit, and two days is not a week. Narrowing the chip to 56px fits three days plus both arrows, which keeps the *same paging interaction at every width*. Decided 2026-09-08 while drawing the hi-fi frames | A horizontally scrollable strip (fits four days, but changes the interaction model at one breakpoint only — and the keyboard model already says arrows move between days); two 76px days (fits, and answers "which day?" with almost no answer) |
| Inside the **calendar**, a refused day is struck through and the two rules are stated once in the footer | ST-03 requires every refused date to carry its reason as text. A 76px strip chip can hold "Too far ahead" on two lines; a 40px calendar cell cannot hold it at all. So the calendar carries the reason at *container* level — "Weekends are closed" and "You can book up to Thu 8 Oct" — and each refused cell carries a **strikethrough**, which is a non-colour, non-textual cue. Decided 2026-09-08 | Per-cell reason text (does not fit at any type size the ramp offers); dimming the cell alone (colour and contrast as the only signal, which NFR-008 forbids); shrinking the reason below `--t-label` (outside the closed type scale) |

## Conflicts and open questions

All four rows are resolved and none changes BRD-001. Row 4 was opened and closed on
...
| 3   | ST-04 offers "the next two working days with desks free", which requires availability for dates the user has not selected. Nothing in BRD-001 provides that, and it may be a meaningful query cost.                                                                                                                                            | ST-04 recovery vs REQ-007's per-date scope | Architect        | **Resolved 2026-09-07 (Joy Joshua, PO/BA) — keep the suggestions.** A full day should hand the employee a next step, not a dead end. **Feasibility is now an `/architect` question, not a design one**: if a multi-day availability lookahead proves expensive, ST-04 degrades to *"Try another day"* with no suggestions and no other state changes. Codified 2026-09-10 as **REQ-035** (Could), which carries that degradation as its stated fallback |
| 4   | **The palette has no destructive *action* colour.** ST-10's only route through is **Cancel A-01 for this date** — a destructive action. `tokens.css` carries a danger *family* of `--c-danger-fill` / `--c-danger-border` / `--c-danger-ink`, but nothing intended to fill a solid destructive button; filling one with `--c-danger-border` would use a border colour as a fill, which is exactly the naming rule the file depends on to stay enforceable. | ST-10 recovery vs `tokens.css` naming rules 1–2 | Designer (pass 2b) | **RESOLVED 2026-09-08 (Joy Joshua, designer) — a SOLID destructive action was added.** `tokens.css` gained `--c-danger-action` / `-hover` / `-pressed` / `-label`, and `Button / Type=Danger` is now solid crimson with a white label. **No new hue entered the palette:** the fill is `red-600` — the crimson already there — so blocked, cancelled and destructive stay one family, and white on it is **7.18:1** (hover 8.47:1, pressed 9.99:1). It is a role named for an action FILL, *not* `--c-danger-border` reused as one, which is the distinction naming rule 2 exists to protect. Dark theme inverts like the brand action does: `red-300` fill, `red-950` label, 5.59:1. Focus keeps its `--bw-2` offset — the forest ring against this fill is 1.34:1 and would otherwise be invisible. As a pass-2b palette change it is the **product team's** to confirm, and this PR is where they do it. Affects SCR-002 ST-07 identically, and is resolved there too |

## Designer handoff

**Two copy lines came from the rules, not from this spec.** ST-10 requires "the explanation
the rules require" without quoting it, so the frames read *"You can hold one desk per
working day, so cancel this one to book a different desk."* — a restatement of BR-001.1 and
BR-001.2 and nothing more. ST-05's two sentences are split across the title and body of the
same block. Both wordings are the BA's to confirm (`/ba`); neither invents a rule.

**Corrected 2026-09-08:** six frames — ST-05 and ST-10 at all three widths — were showing
ST-04's *"Every desk is taken on Wed 9 Sep."* Attaching a text property to a variant means
the **instance** supplies the words, and these instances had never been given any, so they
inherited the component default. See the note in
[`wireframe-rules.md`](../wireframe-rules.md).

**The hi-fi frames for this screen exist.** All twelve states are drawn at 360, 768
and 1280 in *Employee Desk Booking — Design System & Mockups*
(<https://www.figma.com/design/xjFVgBbMrJUl7Ys3EX3Cbn>), named
`HF / SCR-003 · Book a desk / ST-## <state> · <width>`, on the pass-2b palette as
Figma variables in both themes. Every colour, size and space in those frames is a
token; nothing carries a raw value, so the dark theme is a mode switch rather than
a redraw. The greyscale `WF /` frames stay in the wireframe file. Neither file is
what gets approved — this spec's PR is.
