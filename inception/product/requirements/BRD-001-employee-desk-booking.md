...
| ------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------- | -------- |
| NFR-001 | Locale/time   | All booking dates and the "today" boundary use the office local timezone, which is **`Asia/Kolkata`** (IANA name, UTC+05:30, no daylight saving). The zone is configuration, not a literal in code, and has no default — a process started without it must refuse to start rather than fall back to UTC (BR-001.14). | Must     |
...
- **Statement:** When a booking is cancelled by anyone other than its owner — an Admin cancelling on their behalf (REQ-014), or the cascade from deactivating their account (BR-001.18) — **both** the browser push notification (REQ-027) **and** the cancellation email (REQ-024) must state that the office admin cancelled it, rather than reporting the cancellation without a cause. Both channels name the **role**, never the individual administrator.
- **Rationale:** A message reading "Your booking was cancelled" when the employee did not cancel it is alarming, unexplained, and generates support messages. The email carries this further than the push does: push is opt-in and defaults to off (REQ-026), so the email is the version most employees actually receive. Naming the role rather than the person keeps a named colleague out of a message about taking something away, in a message that reaches every affected employee.
- **Examples:** Pass — *"Your desk for Tue 9 Sep was cancelled by your office admin."* Fail — *"Your booking was cancelled."* sent for an admin-initiated cancellation. Fail — *"Your desk was cancelled by Ravi Menon."*
- **Affects:** REQ-024, REQ-027, REQ-014, REQ-030, BR-001.13, BR-001.15, BR-001.18
- **Decided:** push channel 2026-09-07 (Joy Joshua, PO/BA, SCR-004 open question 1 and SCR-008 open question 4), codified here 2026-09-10. Email channel and the role-not-individual limit 2026-09-14 (Joy Joshua, PO/BA, open question #14), codified here 2026-09-14.
- **Variant — the deactivation cascade:** when the cancellation comes from BR-001.18, the email keeps the wording above but **must not invite the recipient to book another desk**. REQ-005 stops a deactivated user signing in, so a rebooking prompt names an action they cannot take; the recipient is typically a leaver. The email must not state that the account was closed — that is an HR communication, not a booking notification (Joy Joshua, PO/BA, 2026-09-14).
- **Self-cancellation is unaffected:** a booking the owner cancelled themselves (REQ-010) names no actor in either channel. The rule exists for cancellations the employee did not perform.
...
| 14  | BR-001.20 makes the **push** alert name the office admin when someone else cancelled an employee's booking. Nothing was decided about the cancellation **email** (REQ-024), which reaches every employee rather than only those who opted in to push (REQ-026 defaults to off). Should the email name the actor too, on both the admin-cancel path (REQ-014) and the deactivation cascade (BR-001.18)? | PO/BA        | **Resolved** — **yes, the email names the actor too** (2026-09-14, Joy Joshua). BR-001.20 now covers both channels and names the **role**, never the individual administrator — naming the person was offered and declined. A variant was decided in the same session: the deactivation-cascade email omits the invitation to book another desk, because REQ-005 has already stopped that person signing in. Recorded in `2026-09-14-timezone-and-cancellation-email.md`. Closes US-029/AC-04, which was drafted as an explicit placeholder awaiting this answer |
| 15  | NFR-001 and BR-001.14 both require the **office local timezone**, but no requirement anywhere named the zone. Raised by the Architect as open question 1 in `inception/architecture/db-design.md` §6 (owner PO/IT), blocking REQ-006, REQ-028, BR-001.14 and US-030; it was never tracked as a BRD question, so this document carried the gap silently. What is the office timezone? | PO/IT        | **Resolved** — **`Asia/Kolkata`** (2026-09-14, Joy Joshua), now named in NFR-001. Fixed UTC+05:30 with no daylight saving, so 08:00 office local is permanently 02:30 UTC and BR-001.14's reminder can run on a fixed UTC schedule — which also removes the zone-aware-scheduler constraint from the deferred hosting choice. Recorded in `2026-09-14-timezone-and-cancellation-email.md`. The Architect closes the matching db-design question |
