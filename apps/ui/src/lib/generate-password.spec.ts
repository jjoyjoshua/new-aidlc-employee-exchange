...
describe('generatePassword (US-021/AC-04, US-022/AC-01, AC-02, AC-04, REQ-033, V-18)', () => {
  it('generates a value satisfying every V-12 rule, across 200 samples (US-022/AC-01)', () => {
    for (let i = 0; i < 200; i++) {
  ...
  it('never contains an ambiguous glyph, across 200 samples (US-022/AC-02, BR-001.12 — dictated, never emailed)', () => {
    for (let i = 0; i < 200; i++) {
      const password = generatePassword();
  ...
  it('is not the same value twice in a row (US-022/AC-04)', () => {
