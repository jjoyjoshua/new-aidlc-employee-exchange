...
1. **Order is fixed, not decided.** `user_profiles.id` cannot reference a row that does not yet exist. Any story writing both systems writes Auth first.
   ...
   **Scoped to creation** ([ADR-012](ADR-012-update-compensation-across-systems.md), 2026-09-20): this item is a consequence of the foreign key needing a row to reference, which only constrains an *insert*. A story that *updates* both systems finds both rows already existing and this item silent — ADR-012 decides that case, and decides it the other way (`user_profiles` first).
